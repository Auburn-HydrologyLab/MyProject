...
Use parm ; use pprime
           ...
           !This part is to check nonnegativity
            if(.not.(ONw(i+1).ge.0))   ONw(i+1)=1e-8    ;  if(.not.(Ow(i+1).ge.0))    Ow(i+1)=1e-8     
            if(.not.(ONsf(i+1).ge.0))  ONsf(i+1)=1e-8   ;  if(.not.(a(i+1).ge.0))     a(i+1)=1e-8      
            if(.not.(ONss(i+1).ge.0))  ONss(i+1)=1e-8   ;  if(.not.(b(i+1).ge.0))     b(i+1)=1e-8      
            if(.not.(Nw(i+1).ge.0))    Nw(i+1)=1e-8     ;  if(.not.(Pw(i+1).ge.0))    Pw(i+1)=1e-8
            if(.not.(Ns1(i+1).ge.0))   Ns1(i+1)=1e-8    ;  if(.not.(Ps1(i+1).ge.0))   Ps1(i+1)=1e-8
            if(.not.(Ns2(i+1).ge.0))   Ns2(i+1)=1e-8    ;  if(.not.(Ps2(i+1).ge.0))   Ps2(i+1)=1e-8
            if(.not.(NO3w(i+1).ge.0))  NO3w(i+1)=1e-8   ;  if(.not.(mw(i+1).ge.0))    mw(i+1)=1e-8
            if(.not.(NO3s1(i+1).ge.0)) NO3s1(i+1)=1e-8  ;  if(.not.(ms(i+1).ge.0))    ms(i+1)=1e-8
            if(.not.(NO3s2(i+1).ge.0)) NO3s1(i+1)=1e-8  ;
      

            !еееееееееееее  A╡ir  ееееееееееееееееееееееее
            if (.not.(LPOCw(i+1).ge.0))		LPOCw(i+1)=1e-8     ;     if (.not.(LPOCs1(i+1).ge.0))	LPOCs1(i+1)=1e-8    ;      if (.not.(LPOCs2(i+1).ge.0))	LPOCs2(i+1)=1e-8    
            if (.not.(RPOCw(i+1).ge.0))		RPOCw(i+1)=1e-8     ;     if (.not.(RPOCs1(i+1).ge.0))	RPOCs1(i+1)=1e-8    ;      if (.not.(RPOCs2(i+1).ge.0))	RPOCs2(i+1)=1e-8    
            if (.not.(DOCw(i+1).ge.0))		DOCw(i+1)=1e-8      ;     if (.not.(DOCs1(i+1).ge.0))	DOCs1(i+1)=1e-8     ;      if (.not.(DOCs2(i+1).ge.0))	DOCs2(i+1)=1e-8       
            if (.not.(CH4w(i+1).ge.0))		CH4w(i+1)=1e-8      ;     if (.not.(CH4s1(i+1).ge.0))	CH4s1(i+1)=1e-8     ;      if (.not.(CH4s2(i+1).ge.0))	CH4s2(i+1)=1e-8     
